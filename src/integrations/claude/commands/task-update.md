...
Typical calls: `in_progress` to start a task (the tree must be clean apart
from the task's own declared scope, `tasks.yaml`, verification state, and
pending journal targets — see `../coordination.md`), `review` once a worker reports done, `completed
