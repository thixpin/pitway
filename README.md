...
flowchart LR
    ...
    D --> E["📝<br/>Task Graph"]
    E --> F["🛠<br/>TDD"]
    F --> G["⏱️<br/>Verification"]
    G --> H["🏎️<br/>Task Commit"]
    G -. 🔄 .-> F
```

**Workflow Lifecycle:**
`Requirement` → `Milestone` → `Contract` → `Human Approval` → `Task Graph` → `TDD` ⇄ `Verification` → `Task Commit`

* **Workflow Enforcement:** Validates state transitions and enforces task write boundaries.
* **Verification Evidence:** Records proof of correctness before advancing states.
