...
- **Core must never import AI-provider code.** Driver material is text assets only, installed by `pitway init` (Claude Code default on, `--no-claude` opt-out; `--opencode` / `--codex` opt-in). The canonical copy of every asset — skills, protocol docs, and the command docs under `src/integrations/common/commands/` — lives once in `src/integrations/common/`; a driver directory `src/integrations/<driver>/` exists only to hold whole-file overrides whose content genuinely differs (today only `src/integrations/claude/commands/`, for Claude Code's `argument-hint` frontmatter). Resolution is a static, hardcoded driver-then-common lookup — never a runtime-loaded plugin system, never dynamic driver registration, never a Core code change, never AI-provider code inside Core.
